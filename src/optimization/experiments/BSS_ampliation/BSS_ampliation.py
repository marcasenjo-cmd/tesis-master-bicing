...
def run_alpha_experiment(N_STATIONS, weights, alpha, experiment_idx, experiment_output_file, 
                     score_combination='exponential', penalty_power=3, required_nodes=None):
    """
...
def run_all_alpha_experiments(n_stations_values, weight_combinations, alpha_values, score_combination, penalty_power, 
    ...
    config_file = f"{experiment_dir}/config_alpha_screening.txt"
        ...
        for n_stations in n_stations_values:
            ...
            if exp_id not in completed_experiments:
                args = (
                    ...
                    experiments_output_file, score_combination, penalty_power, required_nodes
                )
    ...
    print(f"\nRunning {remaining_experiments} experiments sequentially")
...
if __name__ == "__main__":
    ...
    alpha_values = [0.5]
    n_stations_values = [40]

    df_stations = pd.read_csv(f'{project_root}/{PR_EXP}/BSS_ampliation/bicing_stations_no_eixample.csv')
    required_nodes = df_stations['nearest_node_id'].tolist()[:20]
    print("There are", len(required_nodes), "required nodes")
    
    # Create results directory
    RESULTS_DIR = f"{PR_EXP}/BSS_ampliation/"
    os.makedirs(RESULTS_DIR, exist_ok=True)
    RESULTS_FILE = f"{RESULTS_DIR}/alpha_screening.csv"
