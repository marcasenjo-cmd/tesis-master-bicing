...
# Variables que no queremos usar como predictors
MANUAL_DROP_COLS: List[str] = [
    "short_name",
    "nearby_distance",
    "last_updated",
    "post_code",
    "unemployment",  # sigue vacío
]

# Variables operativas derivadas del estado de estaciones: leakage
LEAKAGE_COLS = [
...
def create_oof_predictions(
    ...
    out = full_df.loc[
        y.index,
        [c for c in ["station_id", "name", "neighborhood", TARGET_COL] if c in full_df.columns]
    ].copy()

    for model_name, model in models.items():
        preds = cross_val_predict(model, X, y, cv=cv, n_jobs=1)
        out[f"pred__{model_name}"] = preds
        out[f"residual__{model_name}"] = y.values - preds
        out[f"abs_error__{model_name}"] = np.abs(y.values - preds)
...
def main() -> None:
    ...
    if not HAS_XGBOOST:
        ...
        print("Para instalarlo en tu venv:")
        print(r"python -m pip install xgboost")
